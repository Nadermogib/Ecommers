    ...
    addToCart({productId,quantity:1},
      {
        ...
        onError:(error:any)=>{
         Alert.alert("Error",error?.response?.data?.error || "Filed to add to cart") 
        }
      }
    )
  }
  ...
  const renderProduct=({item:product}:{item:Product})=>(
    ...
    >
      <View className='relative'>
        ...
        <TouchableOpacity
         className='absolute tob-3 right-3 bg-black/30 backdrop-blur-xl p-2 rounded-full'
         activeOpacity={0.7}
  ...
  if(isLoading){
    ...
    return(
      <View className='py-20 items-center justify-center '>
        ...
        <Text className='text-text-primary font-semibold  mt-4'>Filed to load products</Text>
