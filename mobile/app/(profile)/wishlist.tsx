...
const WishlistScreen = () => {
  const {wishlist,isLoading,isRemovingFromWishlst,removeFromWishlist,isError}=useWishlist();
  ...
  const handleAddToCart=(productId:string,productName:string)=>{
    addToCart({productId,quantity:1},{
      ...
      onError:()=>Alert.alert("Erorr","Filed to add to cart")
    })
  }
  ...
  return <SafeScreen>
     {/* HEADER */}
     <View className='px-6 pb-5 border-b mt-2 border-surface flex-row items-center'>
      <TouchableOpacity
      onPress={()=>router.back()}
      className='mr-4 '
      >
     ...
     {wishlist.length===0?(
      <View className='flex-1 items-center justify-center px-6'>
        ...
        <Text className='text-text-secondary text-center mt-2'>
          Start adding product you love
        </Text>
        <TouchableOpacity
        className='bg-primary rounded-2xl px-6 py-6 mt-6'
        activeOpacity={0.8}
            ...
            >
              <View className='flex-row p-4'>
                ...
                <TouchableOpacity
                  ...
                  disabled={isRemovingFromWishlst}
                >
