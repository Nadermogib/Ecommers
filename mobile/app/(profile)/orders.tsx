...
const OrdersScreen = () => {
  ...
  const handleOpenRating=(order:Order)=>{
    ...
    //initial rating for all product to 0
    const initalRating:{[key:string]:number}={}

    order.orderItems.forEach((item)=>{
      const productId=item.product._id;
      initalRating[productId]=0
    })

    setProductRating(initalRating)
  }

  const handleSubmitRating=async()=>{
    if(!selectedOrder) return

    //cheack if all products has been rating
    const allRating=Object.values(productRating).every((ratting)=>ratting>0)
    if(!allRating){
      Alert.alert("Error","please rating all products");
      return
    }
    ...
    try {
      ...
      Alert.alert("Success","Tanks you for rating all products")
